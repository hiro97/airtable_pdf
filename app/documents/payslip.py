...
class PayslipGenerator(BaseDocumentGenerator):
    ...
    def fetch_data(self, record_id: str) -> Dict[str, Any]:
        ...
        return self.transform_fields(fields, record_id)

    def transform_fields(self, fields: dict, record_id: str = "") -> Dict[str, Any]:
        ...
        account_no = self._extract_text(self._find_field_value(fields, ["계좌번호", "계좌 번호", "Account Number", "Account No", "Account"]))

        # Dates
        period = self._extract_text(self._find_field_value(fields, ["귀속월", "귀속 월", "급여월", "Period", "Month"]))
        payment_date = self._extract_text(self._find_field_value(fields, ["지급일자", "지급일", "지급 일자", "Payment Date", "Pay Date", "Date"]))

        # Fallbacks for Name / Period from primary field
        if not name:
            name = self._extract_text(fields.get("Name", ""))
        if not period and "Name" in fields:
            match = re.search(r'\d{4}[-_.]\d{2}', str(fields["Name"]))
            if match:
        ...
        overtime_note = self._extract_text(self._find_field_value(fields, ["수당 비고", "연장근로시간", "수당내역", "연장근로"]))
        if not overtime_note and self._find_field_value(fields, ["연장근로시간"]):
            hours = self._find_field_value(fields, ["연장근로시간"])
            overtime_note = f"연장근로 {hours}시간"
        ...
        year_end_tax = self._extract_number(self._find_field_value(fields, ["연말정산(소득세)", "연말정산 소득세", "Year-end Tax"]))
        year_end_local_tax = self._extract_number(self._find_field_value(fields, ["연말정산(지방소득세)", "연말정산 지방소득세", "Year-end Local Tax"]))
