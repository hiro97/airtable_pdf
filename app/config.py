...
class Settings(BaseSettings):
    ...
    BASE_URL: str = Field(default="http://localhost:8000", description="Coolify 배포 공용 도메인 (예: https://po.yourdomain.com)")
    
    # Webhook Security
    WEBHOOK_SECRET: str = Field(default="", description="웹훅 인증용 비밀 토큰 (Airtable 헤더 또는 쿼리)")

    # Airtable Credentials
    AIRTABLE_API_KEY: str = Field(default="", description="Airtable Personal Access Token")
    AIRTABLE_BASE_ID: str = Field(default="", description="Airtable Base ID (appXXXXXXXXXXXXXX)")

    # Table Names
    PO_TABLE_NAME: str = Field(default="Purchase Orders", description="부모 발주서 테이블 이름")
    ITEMS_TABLE_NAME: str = Field(default="Episodes", description="자식 품목/에피소드 테이블 이름")

    # Parent Table Field Names
    FIELD_CHECKBOX: str = Field(default="Generate PO", description="발주서 생성 체크박스 필드")
    ...
    FIELD_ITEMS_LINK: str = Field(default="Episodes", description="자식 레코드 링크 필드명")
    FIELD_LINGUIST_NAME: str = Field(default="Linguist", description="번역가 이름 필드")
    FIELD_LINGUIST_POSITION: str = Field(default="Position", description="직무/포지션 (예: SDH, TRS)")
    ...
    # Child Table (Episodes/Items) Field Names
